...
function AssociationLevelList() {
    ...
    const columns = useMemo(
        () =>
            getColumns({
                ...
                onEdit,
                userRole,
                searchQuerykey: "associationLevelName",
    ...
    const toolbarAttributes = [
        <Input
            ...
            value={(table.getColumn("associationLevelName")?.getFilterValue() as string) ?? ""}
            onChange={(event) => table.getColumn("associationLevelName")?.setFilterValue(event.target.value)}
    ...
    return (
        <div className="h-full flex-1 flex-col space-y-8 md:flex">
            <div className="flex items-center justify-between space-y-2">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Association Level List</h2>
                    <p className="text-muted-foreground">Here&apos;s a list of association levels.</p>
                </div>
                <div className="flex items-center space-x-2">
                    <ConditionalButton
                        onClick={() => navigator(NAVIGATION_ROUTES.ASSOCIATION_LEVEL_CREATE)}
...
export default AssociationLevelList;
