...
function AgeList() {
  ...
  const fetchList = async () => {
    try {
      setIsLoading(true);
      const response = await MetadataService.getAllAge({});
  ...
  const onDelete = useCallback(async (id: string) => {
    try {
      setIsLoading(true);
      const response = await MetadataService.deleteData(id, "/api/admin/age-range/delete/");
  ...
  const toolbarAttributes = [
    <Input
      ...
      value={(table.getColumn("ageRange")?.getFilterValue() as string) ?? ""}
      onChange={(event) => table.getColumn("ageRange")?.setFilterValue(event.target.value)}
  ...
  return (
    <div className="h-full flex-1 flex-col space-y-8 md:flex">
      <div className="flex items-center justify-between space-y-2">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Age Range List</h2>
          <p className="text-muted-foreground">Here&apos;s a list of age range.</p>
        </div>
        <div className="flex items-center space-x-2">
          <ConditionalButton onClick={() => navigator(NAVIGATION_ROUTES.AGE_CREATE)} accessLevel="super_admin">
            Create Age
          </ConditionalButton>
        </div>
      </div>
