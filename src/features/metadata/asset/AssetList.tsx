...
function AssetList() {
  ...
  const fetchList = async () => {
    try {
      setIsLoading(true);
      const response = await MetadataService.getAllAsset({});
      if (response.status === HTTP_STATUS_CODES.OK) {
        const assets = response.data;
        assets.forEach((asset: team, i: number) => {
          assets[i].createdBy = asset?.createdBy?.email || "N/A";
          assets[i].modifiedBy = asset?.modifiedBy?.email || "N/A";
        });
        setDataList(assets);
      }
    } catch (error) {
  ...
  const onDelete = useCallback(async (id: string) => {
    try {
      setIsLoading(true);
      const response = await MetadataService.deleteData(id, "/api/admin/asset/delete/");
  ...
  const onEdit = useCallback((id: string) => {
    navigate(`${NAVIGATION_ROUTES.ASSET_EDIT}/${id}`);
  }, []);
  ...
  const toolbarAttributes = [
    <Input
      ...
      onChange={(event) => table.getColumn("assetName")?.setFilterValue(event.target.value)}
  ...
  return (
    <div className="h-full flex-1 flex-col space-y-8 md:flex">
      <div className="flex items-center justify-between space-y-2">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Asset List</h2>
          <p className="text-muted-foreground">Here&apos;s a list of assets.</p>
        </div>
        <div className="flex items-center space-x-2">
          <ConditionalButton onClick={() => navigator(NAVIGATION_ROUTES.ASSET_CREATE)} accessLevel="all_staff">
            Create Asset
          </ConditionalButton>
        </div>
      </div>
