...
function MarketingPlatformList() {
    ...
    const fetchList = async () => {
        try {
            setIsLoading(true);
            const response = await MetadataService.getAllMarketingPlatform({});
            if (response.status === HTTP_STATUS_CODES.OK) {
                const platforms = response.data;
                platforms.forEach((platform: team, i: number) => {
                    platforms[i].createdBy = platform?.createdBy?.email || "N/A";
                    platforms[i].modifiedBy = platform?.modifiedBy?.email || "N/A";
                });
                setDataList(platforms);
            }
        } catch (error) {
    ...
    const onDelete = useCallback(async (id: string) => {
        try {
            setIsLoading(true);
            const response = await MetadataService.deleteData(id, "/api/admin/marketing-platform/delete/");
    ...
    const toolbarAttributes = [
        <Input
            ...
            value={(table.getColumn("marketingPlatformName")?.getFilterValue() as string) ?? ""}
            onChange={(event) => table.getColumn("marketingPlatformName")?.setFilterValue(event.target.value)}
        ...
        <div className="h-full flex-1 flex-col space-y-8 md:flex">
            <div className="flex items-center justify-between space-y-2">
                <div>
                    ...
                    <p className="text-muted-foreground">Here&apos;s a list of platforms.</p>
                </div>
                <div className="flex items-center space-x-2">
                    <ConditionalButton
                        onClick={() => navigator(NAVIGATION_ROUTES.MARKETING_PLATFORM_CREATE)}
...
export default MarketingPlatformList;
