...
function CountryList() {
  ...
  const fetchList = async () => {
    try {
      setIsLoading(true);
      const response = await MetadataService.getAllCountry({});
      if (response.status === HTTP_STATUS_CODES.OK) {
        const countries = response.data;
        countries.forEach((country: team, i: number) => {
          countries[i].createdBy = country?.createdBy?.email || "N/A";
          countries[i].modifiedBy = country?.modifiedBy?.email || "N/A";
        });
  ...
  const onDelete = useCallback(async (id: string) => {
    try {
      setIsLoading(true);
      const response = await MetadataService.deleteData(id, "/api/admin/nationality/delete/");
  ...
  const toolbarAttributes = [
    <Input
      ...
      value={(table.getColumn("nationalityName")?.getFilterValue() as string) ?? ""}
      onChange={(event) => table.getColumn("nationalityName")?.setFilterValue(event.target.value)}
    ...
    <div className="h-full flex-1 flex-col space-y-8 md:flex">
      <div className="flex items-center justify-between space-y-2">
        <div>
          ...
          <p className="text-muted-foreground">Here&apos;s a list of countries.</p>
        </div>
        <div className="flex items-center space-x-2">
          <ConditionalButton onClick={() => navigator(NAVIGATION_ROUTES.COUNTRY_CREATE)} accessLevel="super_admin">
            Create Country
          </ConditionalButton>
        </div>
      </div>
