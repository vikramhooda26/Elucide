...
function ParentOrgList() {
  ...
  const fetchList = async () => {
    try {
      setIsLoading(true);
      const response = await MetadataService.getAllParentOrg({});
  ...
  const onDelete = useCallback(async (id: string) => {
    try {
      setIsLoading(true);
      const response = await MetadataService.deleteData(id, "/api/admin/parent-org/delete/");
  ...
  const toolbarAttributes = [
    <Input
      ...
      value={(table.getColumn("parentOrgName")?.getFilterValue() as string) ?? ""}
      onChange={(event) => table.getColumn("parentOrgName")?.setFilterValue(event.target.value)}
  ...
  return (
    <div className="h-full flex-1 flex-col space-y-8 md:flex">
      <div className="flex items-center justify-between space-y-2">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Parent Organization List</h2>
          <p className="text-muted-foreground">Here&apos;s a list of organizations.</p>
        </div>
        <div className="flex items-center space-x-2">
          <ConditionalButton onClick={() => navigator(NAVIGATION_ROUTES.PARENT_ORG_CREATE)} accessLevel="all_staff">
            Create Organization
          </ConditionalButton>
        </div>
      </div>
