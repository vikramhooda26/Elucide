...
function CityList() {
  ...
  const fetchList = async () => {
    try {
      setIsLoading(true);
      const response = await MetadataService.getAllCity({});
      if (response.status === HTTP_STATUS_CODES.OK) {
        const cities = response.data;
        cities.forEach((city: team, i: number) => {
          cities[i].createdBy = city?.createdBy?.email || "N/A";
          cities[i].modifiedBy = city?.modifiedBy?.email || "N/A";
        });
        setDataList(cities);
      }
    } catch (error) {
  ...
  const onDelete = useCallback(async (id: string) => {
    try {
      setIsLoading(true);
      const response = await MetadataService.deleteData(id, "/api/admin/city/delete/");
  ...
  const toolbarAttributes = [
    <Input
      ...
      value={(table.getColumn("cityName")?.getFilterValue() as string) ?? ""}
      onChange={(event) => table.getColumn("cityName")?.setFilterValue(event.target.value)}
  ...
  return (
    <div className="h-full flex-1 flex-col space-y-8 md:flex">
      <div className="flex items-center justify-between space-y-2">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">City List</h2>
          <p className="text-muted-foreground">Here&apos;s a list of cities.</p>
        </div>
        <div className="flex items-center space-x-2">
          <ConditionalButton onClick={() => navigator(NAVIGATION_ROUTES.CITY_CREATE)} accessLevel="all_staff">
            Create City
          </ConditionalButton>
        </div>
      </div>
