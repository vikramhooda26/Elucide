...
function TeamOwnerList() {
  ...
  const fetchList = async () => {
    try {
      setIsLoading(true);
      const response = await MetadataService.getAllTeamOwner({});
    ...
    <div className="h-full flex-1 flex-col space-y-8 md:flex">
      <div className="flex items-center justify-between space-y-2">
        ...
        <div className="flex items-center space-x-2">
          <ConditionalButton onClick={() => navigator(NAVIGATION_ROUTES.TEAM_OWNER_CREATE)} accessLevel="all_staff">
            Create Owner
          </ConditionalButton>
        </div>
      </div>
