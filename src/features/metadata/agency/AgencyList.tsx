...
function AgencyList() {
  ...
  const fetchList = async () => {
    try {
      setIsLoading(true);
      const response = await MetadataService.getAllAgency({});
      if (response.status === HTTP_STATUS_CODES.OK) {
        const agencies = response.data;
        agencies.forEach((agency: team, i: number) => {
          agencies[i].createdBy = agency?.createdBy?.email || "N/A";
          agencies[i].modifiedBy = agency?.modifiedBy?.email || "N/A";
        });
        setDataList(agencies);
      }
    } catch (error) {
  ...
  const onDelete = useCallback(async (id: string) => {
    try {
      setIsLoading(true);
      const response = await MetadataService.deleteData(id, "/api/admin/agency/delete/");
  ...
  const toolbarAttributes = [
    <Input
      ...
      value={(table.getColumn("agencyName")?.getFilterValue() as string) ?? ""}
      onChange={(event) => table.getColumn("agencyName")?.setFilterValue(event.target.value)}
    ...
    <div className="h-full flex-1 flex-col space-y-8 md:flex">
      <div className="flex items-center justify-between space-y-2">
        <div>
          ...
          <p className="text-muted-foreground">Here&apos;s a list of agencies.</p>
        </div>
        <div className="flex items-center space-x-2">
          <ConditionalButton onClick={() => navigator(NAVIGATION_ROUTES.AGENCY_CREATE)} accessLevel="all_staff">
            Create Agency
          </ConditionalButton>
        </div>
      </div>
