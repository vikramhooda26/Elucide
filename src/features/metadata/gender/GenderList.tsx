...
function GenderList() {
  ...
  const fetchList = async () => {
    try {
      setIsLoading(true);
      const response = await MetadataService.getAllGender({});
  ...
  const onDelete = useCallback(async (id: string) => {
    try {
      setIsLoading(true);
      const response = await MetadataService.deleteData(id, "/api/admin/gender/delete/");
  ...
  const toolbarAttributes = [
    <Input
      ...
      value={(table.getColumn("genderName")?.getFilterValue() as string) ?? ""}
      onChange={(event) => table.getColumn("genderName")?.setFilterValue(event.target.value)}
  ...
  return (
    <div className="h-full flex-1 flex-col space-y-8 md:flex">
      <div className="flex items-center justify-between space-y-2">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Gender List</h2>
          <p className="text-muted-foreground">Here&apos;s a list of genders.</p>
        </div>
        <div className="flex items-center space-x-2">
          <ConditionalButton onClick={() => navigator(NAVIGATION_ROUTES.GENDER_CREATE)} accessLevel="super_admin">
            Create Gender
          </ConditionalButton>
        </div>
      </div>
