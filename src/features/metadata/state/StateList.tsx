...
function StateList() {
  ...
  const fetchList = async () => {
    try {
      setIsLoading(true);
      const response = await MetadataService.getAllState({});
      if (response.status === HTTP_STATUS_CODES.OK) {
        const states = response.data;
        states.forEach((state: team, i: number) => {
          states[i].createdBy = state?.createdBy?.email || "N/A";
          states[i].modifiedBy = state?.modifiedBy?.email || "N/A";
        });
        setDataList(states);
      }
    } catch (error) {
  ...
  const toolbarAttributes = [
    <Input
      ...
      value={(table.getColumn("stateName")?.getFilterValue() as string) ?? ""}
      onChange={(event) => table.getColumn("stateName")?.setFilterValue(event.target.value)}
    ...
    <div className="h-full flex-1 flex-col space-y-8 md:flex">
      <div className="flex items-center justify-between space-y-2">
        <div>
          ...
          <p className="text-muted-foreground">Here&apos;s a list of states.</p>
        </div>
        <div className="flex items-center space-x-2">
          <ConditionalButton onClick={() => navigator(NAVIGATION_ROUTES.STATE_CREATE)} accessLevel="super_admin">
            Create State
          </ConditionalButton>
        </div>
      </div>
