...
function TierList() {
  ...
  const fetchList = async () => {
    try {
      setIsLoading(true);
      const response = await MetadataService.getAllTier({});
      if (response.status === HTTP_STATUS_CODES.OK) {
        const tiers = response.data;
        tiers.forEach((tier: team, i: number) => {
          tiers[i].createdBy = tier?.createdBy?.email || "N/A";
          tiers[i].modifiedBy = tier?.modifiedBy?.email || "N/A";
        });
        setDataList(tiers);
      }
    } catch (error) {
  ...
  const onDelete = useCallback(async (id: string) => {
    try {
      setIsLoading(true);
      const response = await MetadataService.deleteData(id, "/api/admin/tier/delete/");
  ...
  const toolbarAttributes = [
    <Input
      ...
      value={(table.getColumn("tierName")?.getFilterValue() as string) ?? ""}
      onChange={(event) => table.getColumn("tierName")?.setFilterValue(event.target.value)}
  ...
  return (
    <div className="h-full flex-1 flex-col space-y-8 md:flex">
      <div className="flex items-center justify-between space-y-2">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Tier List</h2>
          <p className="text-muted-foreground">Here&apos;s a list of tiers.</p>
        </div>
        <div className="flex items-center space-x-2">
          <ConditionalButton onClick={() => navigator(NAVIGATION_ROUTES.TIER_CREATE)} accessLevel="super_admin">
            Create Tier
          </ConditionalButton>
        </div>
      </div>
