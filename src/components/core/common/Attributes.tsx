import { Diamond, MoveHorizontal, PersonStanding, SquareStack, Unlink2 } from 'lucide-react';
            ...
            <Card className="overflow-hidden">
                ...
                <CardContent className="p-6 text-sm">
                    <div className="grid gap-3">
                        ...
                        {data?.owners?.length > 0 ?
                            <>
                                ...
                                <ul className="grid gap-3">
                                    {data?.owners?.map((owner: string, i: number) => (
                        ...
                        <ul className="grid gap-3">
                            {data?.nccs?.length > 0 ? data?.nccs?.map((nccs: string, i: number) => (
                                <li key={i} className="flex items-center gap-2">
                                    <span className="text-muted-foreground">
                    ...
                    <div className="grid grid-cols-2 gap-4 my-4">
                        ...
                        {data?.personalityTraits?.length
                            ? data?.personalityTraits?.map((trait: any, i: number) => (
                                <>
                                    ...
                                    <div key={i + 100} className="grid auto-rows-max gap-3">
                                        <div className="text-muted-foreground">
                ...
                <CardFooter className="flex flex-row items-center border-t bg-muted/50 px-6 py-3">
                    <div className="text-xs text-muted-foreground">
                        Updated <time dateTime="2023-11-23">August 1, 2024</time>
                    </div>
                    ...
                    <CardContent className="p-6 text-sm">
                        <div className="grid gap-3">
                            <div className="grid gap-3">
                                ...
                                <dl className="grid gap-3">
                                    {typeof data?.age === 'object' ?
                                        data?.age?.length ?
                                            data?.age?.map((age: string, i: number) => (
                                                <>
                                                    {age ?
                                                        <div key={i} className="flex items-center gap-2">
                                                            <dt className="">
                            ...
                            <div className="grid gap-3">
                                ...
                                <dl className="grid gap-3">
                                    {data?.gender?.length
                                        ? data?.gender?.map((gender: string, i: number) => (
                                            <>
                                                {gender ?
                                                    <div key={i} className="flex items-center gap-2">
                                                        <dt className="">
