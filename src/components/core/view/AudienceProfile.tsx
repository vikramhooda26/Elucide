...
function AudienceProfile({ data, title = "" }: Props) {
  ...
  const sortedAgeRange = data?.age
    ? Array.from(data?.age as any[]).sort((a: any, b: any) => (a?.name && b?.name ? a.name.localeCompare(b.name) : 0))
    : null;

  const sortedNccsClass = data?.nccs
    ? Array.from(data?.nccs as any[]).sort((a: any, b: any) => (a?.name && b?.name ? a.name.localeCompare(b.name) : 0))
    : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card x-chunk="dashboard-07-chunk-0" className="overflow-hidden">
        <CardHeader className="flex flex-row items-start bg-muted/50">
          <div className="grid gap-0.5">
            <CardTitle className="group flex items-center gap-2 text-lg">{title || ""} Characteristics</CardTitle>
          </div>
        </CardHeader>
                      ...
                      <div className="grid">
                        <div key={i} className="grid">
                          <div className="flex flex-col text-muted-foreground">
                            ...
                            <span>{calculatePercentages(trait, i === data?.mainPersonalityTraits?.length - 1)}%</span>
                          </div>
                        </div>
                      </div>
