...
type Props = {
  ...
  values?: {
    value1: number[] | readonly number[];
    value2: number[] | readonly number[];
...
function DoubleRangeWithCheckFilter({
  ...
  const handleOperationChange = (operationType: string) => {
    ...
    if (operationType === "in") {
      ...
      onValueChange({
        value1: singleBoundValue?.value1 as number[],
        value2: singleBoundValue?.value2 as number[],
        operationType,
        checkType: values?.checkType || ""
      });
    }
  ...
  const handleCheckTypeChange = (checkType: string) => {
    ...
    onValueChange({
      value1: values?.value1 as number[],
      value2: values?.value2 as number[],
      operationType: values?.operationType || "",
      checkType
    });
  };
