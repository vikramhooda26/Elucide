...
class OptionalColumns {
    static getOptionalColumns(
        ...
        const columnConfigsMapFilter: Array<{
            ...
            title: string;
            isMandatory: boolean;
        }> = columnConfigs?.filter((d) => d?.filterKey && optionalFilterKeys.includes(d?.filterKey))
            ?.map((config) => {
        ...
        const columns: CustomColumnDef<TSchemaType>[] = columnConfigsMapFilter.map((config) =>
            createDynamicColumn<TSchemaType>(config.accessorKey as keyof TSchemaType, config.filterKey, config.title, config?.isMandatory)
        );
